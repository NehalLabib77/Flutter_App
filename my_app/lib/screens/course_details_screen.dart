...
class _CourseDetailsScreenState extends State<CourseDetailsScreen> {
  ...
  Future<void> _enroll() async {
    ...
    final ok = await requireLogin(
      context,
      action: c.isFree ? 'enroll in free courses' : 'enroll in this paid course',
    );
    if (!ok) return;
    if (!mounted) return;
    ...
    final amount = _parseAmount(c.price);
    if (!c.isFree && amount > 0) {
      ...
      try {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not save enrollment: $e')),
        );
        return;
      }
    }
